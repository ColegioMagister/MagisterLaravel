                                ...
                                <tbody>
                                    ...
                                    @foreach ($school_periods as $school_period)
                                        <tr>
                                            ...
                                            <td class="align-middle text-uppercase text-sm ">{{ $school_period->end_date }}
                                            </td>

                                            <td>
                                                <span
                                                    class="badge {{ $school_period->status ? 'bg-success' : 'bg-danger' }}">
                                                    {{ $school_period->status ? '' : '' }}
                                                </span>
                                            </td>

                                            <td class="align-middle text-uppercase text-sm">
                                                <a href="{{ route('schoolYear.show', $school_period->id) }}"
                                                    class="btn btn-success">
                                                    <i class="fa-solid fa-eye fa-xl"></i> &nbsp;Ver</a>


                                                <button type="submit" class="btn btn-primary" data-bs-toggle="modal"
                                                    data-bs-target="#EditarAñoEs"
                                                    data-url='{{ route('school_periods.edit', $school_period->id) }}'
                                                    data-send="{{ route('school_periods.ajax.edit', $school_period) }}"
                                                    enctype="multipart/form-data">
                                                    <i class="fa-solid fa-pencil fa-xl"></i> &nbsp; Editar
                                                </button>

                                                <form class="alertDelete" method="POST"
                                                    action="{{ route('school_periods.delete', $school_period->id) }}"
    ...
    <div class="modal fade" id="ModRegAño" tabindex="-1" aria-labelledby="ModRegAño" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <!-- #para que cargue  -->
                <form role="form" class="text-start" method="POST" action="{{ route('schoolYear.store') }}"
                    enctype="multipart/form-data">
                    {!! csrf_field() !!}
    ...
    <div class="modal fade" id="EditarAñoEs" tabindex="-1" aria-labelledby="EditarAñoEs" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form role="form" id='edit-school_period-form' class="text-start alertEdits" method="POST"
                        ...
                        <div class="input-group input-group-outline mt-2 mb-4">
                            <div class="col-6">
                                <div class="input-group input-group-outline me-2 is-focused">
                                    ...
                                    <select class="form-control" name="period_name">
                                        <option class="period_select" selected disabled value="">Codigo de Periodo
                                        </option>
                                        @foreach ($periodo as $per)
                                            <option value="{{ $per }}">{{ $per }}</option>
                                        @endforeach
